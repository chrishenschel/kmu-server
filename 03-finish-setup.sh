...
" 2>/dev/null)
    ...
    if [ $? -eq 0 ] && [ -n "$TOKEN_KEY" ]; then
        ...
        echo "Waiting for Stalwart and LDAP outpost to be ready..."
        ...
        echo "Triggering account creation and promoting $STALWART_USER..."
        for i in $(seq 1 10); do
            ...
            if echo "$RESULT" | grep -q "error"; then
                echo "  Attempt $i/10 - account not ready yet, retrying..."
                sleep 5
            else
