...
| Stalwart: `No TLS certificates available (tls.no-certificates-available)` | Stalwart reads TLS certs from Caddy's data dir; certs may not exist yet or path may be wrong. | Ensure `stalwart/data/etc/config.toml` has the real domain (no `__DOMAIN__`). Trigger Caddy to obtain the cert by opening `https://mail.<your-domain>` once, wait ~30s, then run `docker compose restart stalwart-mail`. Cert path in config must be `.../caddy-data/certificates/acme-v02.../mail.<domain>/...` (no `caddy/` in the path). |
