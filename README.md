...
| Stalwart: user shows 0 aliases / mail client "wrong password" | With LDAP, the user's email comes from Authentik's **mail** attribute. If it's empty, Stalwart shows "0 aliases" and IMAP may fail. Some clients send full email as username; Stalwart's name filter must match. | In **Authentik** → Directory → Users → open the user → set **Email** to e.g. `test@<domain>`. Save. Restart Stalwart or wait for LDAP cache refresh. For IMAP, use **username** `test` (or full email `test@<domain>` if the config uses the `(|(cn=?)(mail=?))` name filter). Password = the user's Authentik password. |
