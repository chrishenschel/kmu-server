...
| Stalwart: user shows 0 aliases / mail client "wrong password" | With LDAP, the user's email comes from Authentik's **mail** attribute. If it's empty, Stalwart shows "0 aliases" and IMAP may fail. Stalwart's LDAP name filter in `stalwart/data/etc/config.toml` defaults to `name = "(cn=?)"`, so logins must use the **username** (not the email address). | In **Authentik** → Directory → Users → open the user → set **Email** to e.g. `test@<domain>`. Save. Restart Stalwart or wait for LDAP cache refresh so the alias appears. For IMAP/SMTP login, use **username** `test` (not `test@<domain>`). Password = the user's Authentik password. |
