...
| Admin panel: user created but not visible in Stalwart | Admin panel creates the mailbox via Stalwart API using `admin` + `PASSWORD` from `.env`. If that doesn't match Stalwart's fallback-admin password, the API returns 401. **With Stalwart directory = LDAP, the API cannot create principals.** The LDAP provider's **search group** limits which users are visible to Stalwart: only users in that group can log in to mail. The blueprint uses the group **"Stalwart Mail Users"**; new users created via the admin panel are added to that group so they can authenticate. | **New installs:** Run the updated setup so the "Stalwart Mail Users" group exists and the LDAP provider uses it; create users via the admin panel so they're added to the group. **Existing installs:** In Authentik create a group "Stalwart Mail Users", add your initial admin and the new user to it, then edit the **Stalwart LDAP** provider and set **Search group** to "Stalwart Mail Users". Restart authentik-ldap if needed. |
...
| Stalwart: user shows 0 aliases / mail client "wrong password" | With LDAP, the user's email comes from Authentik's **mail** attribute. If it's empty, Stalwart shows "0 aliases" and IMAP may fail. Stalwart's LDAP name filter in `stalwart/data/etc/config.toml` is `name = "(|(cn=?)(mail=?))"`, so logins work with either the **username** or the full email address. | In **Authentik** → Directory → Users → open the user → set **Email** to e.g. `test@<domain>`. Save. Restart Stalwart or wait for LDAP cache refresh so the alias appears. For IMAP/SMTP login, use either **username** `test` or full email `test@<domain>`. Password = the user's Authentik password. |
| Stalwart: "Mail server error" when sending (e.g. to self or others) | Stalwart must have `session.rcpt.directory` set so it accepts mail for local LDAP users; otherwise it rejects with "Relay not allowed". Some clients use port **587** for sending; that port must be open. | Ensure `stalwart/data/etc/config.toml` has `[session.rcpt]` with `directory = "authentik"`. Expose port **587** in `docker-compose.yaml` for Stalwart and use **587** (STARTTLS) or **465** (SMTPS) in the mail client for outgoing mail. Restart Stalwart after config changes. Check `docker compose logs stalwart-mail` (or logs under `stalwart/data/logs/`) for SMTP errors. |
